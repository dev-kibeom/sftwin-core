...
TEST(EvaluateTelemetryFailsafeUseCaseTest, ThrowsExceptionAndEngagesRelayOnViolation) {
    ...
    FailsafeRule rule{};
    rule.vision_critical_distance_m = 0.5f;

    EvaluateTelemetryFailsafeUseCase usecase{interlock_mgr, mock_hw, mock_pub, rule};

    VisionDetectionDto intruder{"PERSON", 0.9f, 0.3f, {}};
    EvaluateTelemetryFailsafeRequestDto req{
        TelemetryPacketDto{"ROBOT_01", 1'000'000'000ULL, {10.0f}, {10.0f}, 0.0f},
        {intruder}
    };
    ...
    EXPECT_EQ(mock_pub->last_action, "ESTOP");
}
