...
describe('SceneGraphManager 단위 테스트', () => {
    ...
    describe('Happy Path: 에셋 마운트, 언마운트 및 트랜스폼/관절 갱신', () => {
        ...
        it('Given: 마운트된 로봇 모델이 있을 때, When: updatePose를 호출하면, Then: ROS 좌표가 Three.js 좌표로 변환되어 루트 모델의 위치와 회전이 갱신되어야 한다.', () => {
            const assetId = 'amr_01';
            const model = new THREE.Group();
            sceneGraphManager.mountAsset(assetId, model);

            // When: ROS TF (x=5, y=3, z=1)
            sceneGraphManager.updatePose(assetId, {
                position: [5, 3, 1],
                rotation: [0, 0, 0, 1],
            });

            // Then: Three.js x = -3, y = 1, z = -5
    ...
    describe('Edge Cases & Error Handling', () => {
        ...
        it('Given: 존재하지 않는 assetId에 대해 updateJoints 또는 updatePose를 호출할 때, When: 함수를 실행하면, Then: 크래시 없이 안전하게 무시되어야 한다.', () => {
            expect(() =>
                sceneGraphManager.updateJoints('non_existent_id', { joint_1: 1.0 })
            ).not.toThrow();

            expect(() =>
                sceneGraphManager.updatePose('non_existent_id', {
                    position: [0, 0, 0],
                    rotation: [0, 0, 0, 1],
                })
            ).not.toThrow();
